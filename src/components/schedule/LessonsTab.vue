<template>
    <div>
        <div class="d-flex justify-space-between align-center mb-6">
            <h2 class="text-h5">Lista zajęć</h2>
            <v-btn color="primary" variant="tonal" @click="initNewLesson()">
        ...
        <v-card class="mb-6">
            <v-card-text>
                <v-row>
                    ...
                    <v-col cols="12" md="3">
                        <v-select v-model="groupFilter" :items="groupOptions" label="Grupa" variant="outlined"
                            density="compact" hide-details clearable></v-select>
                    </v-col>
                    <v-col cols="12" md="3">
                        <v-menu>
                            <template v-slot:activator="{ props }">
                                <v-text-field v-bind="props" v-model="dateFromFormatted" label="Data od"
                                    variant="outlined" density="compact" hide-details prepend-inner-icon="mdi-calendar"
                                    readonly></v-text-field>
                            </template>
                            <v-date-picker v-model="dateFrom" locale="pl"
                                @update:model-value="updateDateFrom"></v-date-picker>
                        </v-menu>
                    </v-col>
                    <v-col cols="12" md="3">
                        <v-menu>
                            <template v-slot:activator="{ props }">
                                <v-text-field v-bind="props" v-model="dateToFormatted" label="Data do"
                                    variant="outlined" density="compact" hide-details prepend-inner-icon="mdi-calendar"
                                    readonly></v-text-field>
                            </template>
                            <v-date-picker v-model="dateTo" locale="pl"
                                @update:model-value="updateDateTo"></v-date-picker>
                        </v-menu>
                    </v-col>
                </v-row>
            </v-card-text>
        ...
        <!-- Lista zajęć -->
        <v-card>
            <v-data-table :headers="headers" :items="filteredLessons" item-value="id">
                ...
                <template #item.room="{ item }">
                    <v-chip v-if="item.room" color="info" size="small" variant="outlined">
                ...
                <template #item.group="{ item }">
                    <v-chip v-if="item.group" color="primary" size="small" variant="tonal">
                        {{ item.group.code }}
                    </v-chip>
                    <span v-else class="text-grey">-</span>
                </template>
                ...
                <template #item.lesson_type="{ item }">
                    <v-chip :color="getTypeColor(item.lesson_type)" size="small" variant="outlined">
                        {{ getTypeLabel(item.lesson_type) }}
                    </v-chip>
                </template>

                <template #item.actions="{ item }">
                    <v-icon class="me-2" size="small" @click="editLesson(item)">
                        mdi-pencil
                    </v-icon>
                    <v-icon size="small" @click="deleteLesson(item)" color="error">
                        mdi-delete
                    </v-icon>
                </template>
            </v-data-table>
        ...
        <lesson-dialog v-model="showAddDialog" :lesson="editedLesson" @save="onLessonSaved" @close="closeDialog" />
...
export default {
    ...
    mounted() {
        // Only fetch if stores are empty to avoid redundant calls, or let the parent handle it? 
        // Parent keeps tabs alive? 
        // Let's safe-fetch
        if (this.lessonsStore.lessons.length === 0) this.lessonsStore.fetchLessons()
        if (this.groupsStore.groups.length === 0) this.groupsStore.fetchGroups()
        if (this.programsStore.programs.length === 0) this.programsStore.fetchPrograms()
        if (this.subjectsStore.subjects.length === 0) this.subjectsStore.fetchSubjects()
        if (this.usersStore.allUsers.length === 0) this.usersStore.fetchUsers()
        if (this.roomsStore.rooms.length === 0) this.roomsStore.fetchRooms()
    }
}
</script>
